...
class CycleGANModel(BaseModel):
    ...
    def forward(self):
        ...
        self.fake_B = self.netG_A(self.real_A)  # G_A(A) raw to recap
        self.rec_A = self.netG_B(self.fake_B)  # G_B(G_A(A))
        self.fake_A = self.netG_B(self.real_A)  # G_B(B) raw to more raw
        self.rec_B = self.netG_A(self.fake_A)  # G_A(G_B(B)) recap that
    ...
    def backward_D_B(self):
        ...
        self.loss_D_B = self.backward_D_basic(self.netD_B, self.real_A, rec_B)
    ...
    def backward_G(self):
        ...
        # GAN loss D_B(G_B(B))
        self.loss_G_B = self.criterionGAN(self.netD_B(self.rec_B), True)
        # Forward cycle loss || G_B(G_A(A)) - A||
        self.loss_cycle_A = self.criterionCycle(self.fake_B, self.real_B) * lambda_A
        # Backward cycle loss || G_A(G_B(B)) - B||
        self.loss_cycle_B = self.criterionCycle(self.rec_B, self.real_A) * lambda_B
        # combined loss and calculate gradients
        self.loss_G = (
            self.loss_G_A
            + self.loss_G_B
