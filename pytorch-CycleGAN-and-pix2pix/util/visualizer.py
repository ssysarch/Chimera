...
def save_images2(
    ...
    for label, im_data in visuals.items():
        if label == "real_A" or label == "real_B" or label == "fake_B":
            continue
        print(label)
        im = util.tensor2im(im_data)
        save_path = image_path[0].replace(
            "/home/seongbin/recap-det/data2/fakeface", image_dir
        )
        # save_path = image_path[0].replace(
        #     "/home/seongbin/recap-det/data/CNN_synth_testset/stylegan2", image_dir
        # )
        # save_path = "results/raw2recap/test_20/images/fe.png"
        ...
        # before saving, remove the padding
        # pad = 90
        # im_data = im_data[:, :, pad:-pad, pad:-pad]

        vutils.save_image(im_data, save_path)
        # open again with pillow to sharpen the image
        # im = Image.open(save_path)
        # enhancer = ImageEnhance.Sharpness(im)
        # enhanced = enhancer.enhance(7)
