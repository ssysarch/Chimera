...
class ForgeryAwareAdapterLayer(nn.Module):
    def __init__(
        ...
        # self.down_proj
        self.first_conv_layer = nn.Conv1d(
        ...
        # self.up_proj
        self.second_conv_layer = nn.Conv1d(
    ...
    def forward(self, x):
        ...
        img_patchs = (
            x[1:]
            .transpose(0, 1)
            .reshape(bs, int(math.sqrt(nq)), int(math.sqrt(nq)), md)
            .permute(0, 3, 1, 2)
        )
        dwt_img_l, dwt_img_h = self.dwt_transform(img_patchs)
        dwt_img = torch.cat(
            [dwt_img_l[:, :, None], dwt_img_h[0]], dim=2
        )  # torch.Size([3, 1024, 4, 8, 8])
        hh, ww = dwt_img.shape[-2:]
        dwt_img = dwt_img.flatten(-2)
        dwt_img = self.dwt_norm(dwt_img).permute(
            0, 2, 3, 1
        )  # torch.Size([3, 1024, 4, 64]) -> torch.Size([3, 4, 64, 1024])
        dwt_feature = self.forward_freq(dwt_img)
        dwt_feature = dwt_feature.reshape(bs, 4, hh, ww, md).permute(0, 4, 1, 2, 3)
        dwt_feature = (
            self.idwt_transform((dwt_feature[:, :, 0], [dwt_feature[:, :, 1:]]))
            .flatten(-2)
            .permute(2, 0, 1)
        )
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.resblocks = nn.Sequential(
            *[
                ResidualAttentionBlock(width, heads, attn_mask, add_adapter[idx])
...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        # vit adapter num
        num_vit_adapter = 3  # Args.num_vit_adapter
        add_adapter = [False] * layers
        if num_vit_adapter > 0:
            assert (
                layers % num_vit_adapter == 0
            ), f"layers {layers}, while num_vit_adapter {num_vit_adapter}!"
