...
def validate(model, loader, find_thres=False):
    ...
    with torch.no_grad():
        ...
        for img, label, filen in loader:
            in_tens = img.cuda()
            if opt.arch.startswith("FAT"):
                y_pred.extend(model(in_tens).softmax(dim=1)[:, 1].flatten().tolist())
            else:
                y_pred.extend(model(in_tens).sigmoid().flatten().tolist())
            y_true.extend(label.flatten().tolist())

            filenames.extend(filen)
    ...
    for i in range(len(y_true)):
        if y_true[i] == 1 and y_pred[i] < 0.5:
            false_positive_paths.append(filenames[i])

    with open(os.path.join(res_dir, "false_positive_paths.txt"), "w") as f:
        for item in false_positive_paths:
            f.write("%s\n" % item)
...
class RealFakeDataset(Dataset):
    def __init__(
        ...
        # = = = = = =  transform = = = = = = = = = #
        self.set_transform(arch)

    def set_transform(self, arch):
        stat_from = (
            "imagenet"
            if arch.lower().startswith("imagenet") or arch.startswith("FAT")
            else "clip"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_mode", type=str, default="ours", help="wang2020 or ours"
    )
    parser.add_argument(
        "--max_sample",
        type=int,
    ...
    # # remove file name from opt.result_folder
    # res_dir = os.path.dirname(opt.result_folder)
    res_dir = opt.result_folder
    os.makedirs(res_dir, exist_ok=True)

    model = get_model(opt.arch)
    if opt.arch.startswith("FAT"):
        model.load_state_dict(torch.load(opt.ckpt, map_location="cpu")["model"])
    else:
    ...
    for dataset_path in dataset_paths:
        ...
        print("Dataset loaded... Length of dataset: %d" % (len(dataset)))

        loader = torch.utils.data.DataLoader(
            dataset, batch_size=opt.batch_size, shuffle=False, num_workers=4
        )
        # atk = torchattacks.PGD(model)
        # atk2 = torchattacks.BIM(model)
        # atk.set_normalization_used(mean=MEAN["imagenet"], std=STD["imagenet"])

        # atk.save(loader, save_path="./data1.pt", verbose=True)
        # atk2.save(loader, save_path="./data2.pt", verbose=True)
        # exit()
        ...
        with open(os.path.join(res_dir, "ap.txt"), "a") as f:
            f.write(dataset_path["key"] + ": " + str(round(ap * 100, 2)) + "\n")

        with open(os.path.join(res_dir, "acc0.txt"), "a") as f:
            f.write(
                dataset_path["key"]
                + ": "
